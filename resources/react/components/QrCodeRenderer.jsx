...
export default function QrCodeRenderer({ value, settings = {} }) {
    ...
    useEffect(() => {
        ...
        // READ THE ACTIVE DROPDOWN TOGGLES DIRECTLY FROM YOUR FORM
        ...
        try {
                ...
                for (let c = 0; c < matrixSize; c++) {
                    ...
                    if (isFinderPattern) {
                        ...
                        if (isOuterBorder && cornerSquareStyle === 'outline') {
                            // Draws clean circular hollow outline rings when "Outline Framework" is chosen
                            ctx.beginPath();
                            ctx.arc((c + 0.5) * cellSize, (r + 0.5) * cellSize, cellSize * 0.45, 0, 2 * Math.PI);
                            ctx.fill();
                        } else if (isCenterBlock && cornerDotStyle === 'dots') {
                            // Draws clean circular target points when "Dots / Circles" is chosen
                            ctx.beginPath();
                            ctx.arc((c + 0.5) * cellSize, (r + 0.5) * cellSize, cellSize * 0.45, 0, 2 * Math.PI);
                            ctx.fill();
                        } else {
                        ...
                        if (dotType === 'rounded') {
                            // Draws absolute clean round circles when "Rounded" dot type is chosen
                            ctx.beginPath();
            ...
            QRCode.toDataURL(String(value), {
                ...
                color: { dark: symbolColor, light: "#FFFFFF" },
                errorCorrectionLevel: "H"
            }).then(url => setRenderedDataUrl(url)).catch(e => console.error(e));
        }
